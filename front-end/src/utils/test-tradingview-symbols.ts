...
import { getTradingViewSymbol, NIGERIAN_STOCK_SYMBOLS, getAlternativeSymbolFormats } from '@/components/TradingViewWidget';
...
export function testSymbolConversion(symbol: string): void {
  ...
  console.log(`Found in mapping: ${symbol.toUpperCase() in NIGERIAN_STOCK_SYMBOLS}`);
  console.log(`Alternatives: ${alternatives.join(', ')}`);
}
