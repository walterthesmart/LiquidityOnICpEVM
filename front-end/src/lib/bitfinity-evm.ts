...
import { BITFINITY_NETWORKS, CONTRACT_ADDRESSES, DEFAULT_NETWORK } from './bitfinity-config';
...
export class BitfinityEVMService {
  ...
  async getTokenAddress(symbol: string): Promise<Address | null> {
    try {
      ...
      if (!factoryAddress) {
        throw new Error(`Factory address not configured for network: ${this.network}`);
      }
