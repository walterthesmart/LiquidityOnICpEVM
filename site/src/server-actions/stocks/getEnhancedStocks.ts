 ...
 */
export async function getEnhancedStockBySymbolAction(symbol: string): Promise<EnhancedStockData | null> {
  try {
    if (!symbol) {
 ...
 */
export async function getPendingHederaStocksAction(): Promise<EnhancedStockData[]> {
  try {
...
export async function getStocksByDeploymentStatusAction() {
  try {
    return getStocksByDeploymentStatus();
  } catch (error) {
 ...
 */
export async function getStocksBySectorWithHederaInfoAction() {
  try {
    return getStocksBySectorWithHederaInfo();
  } catch (error) {
...
export async function searchEnhancedStocksAction(
  ...
  try {
    return searchEnhancedStocks(query, filters);
  } catch (error) {
 ...
 */
export async function getEnhancedStockPrice(symbol: string): Promise<EnhancedStockPrice | null> {
  try {
    if (!symbol) {
      throw new Error('Symbol is required');
    }
 ...
 */
export async function getAllEnhancedStockPrices(): Promise<EnhancedStockPrice[]> {
  try {
 ...
 */
export async function getHederaDeployedStockPrices(): Promise<EnhancedStockPrice[]> {
  try {
  ...
  try {
    if (symbol) {
      ...
      return {
        stock: stockData,
        price: priceData,
        isHederaDeployed: stockData?.isHederaDeployed || false,
        hederaTokenId: stockData?.hederaTokenAddress,
        hederaExplorerUrl: stockData?.hederaExplorerUrl
      };
    } else {
      // Get all stocks data
...
export async function getMarketOverviewWithHedera() {
  try {
    const [
      ...
      getStocksBySectorWithHederaInfoAction()
    ]);
    
    // Calculate market statistics
    const totalMarketCap = allPrices.reduce((sum, stock) => sum + (stock.marketCap || 0), 0);
    const hederaMarketCap = hederaPrices.reduce((sum, stock) => sum + (stock.marketCap || 0), 0);
    const totalVolume = allPrices.reduce((sum, stock) => sum + (stock.volume || 0), 0);
    
    const advancers = allPrices.filter(stock => (stock.change || 0) > 0).length;
    const decliners = allPrices.filter(stock => (stock.change || 0) < 0).length;
    const unchanged = allPrices.filter(stock => (stock.change || 0) === 0).length;
