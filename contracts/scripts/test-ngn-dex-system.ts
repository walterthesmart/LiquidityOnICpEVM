...
async function main() {
  ...
  testResults.push(
    await runTest("NGN Stablecoin - Basic Info", async () => {
  ...
  // Test 2: NGN Minting
  testResults.push(
    await runTest("NGN Stablecoin - Minting", async () => {
  ...
  // Test 3: NGN Transfer
  testResults.push(
    await runTest("NGN Stablecoin - Transfer", async () => {
  ...
  testResults.push(
    await runTest("DEX - Get Trading Pairs", async () => {
  ...
  let testStockToken = "";
  testResults.push(
    await runTest("DEX - Get Swap Quote (NGN to Stock)", async () => {
      const allTokens = await dexContract.getAllStockTokens();
  ...
  testResults.push(
    await runTest("DEX - Execute Swap (NGN to Stock)", async () => {
      if (!testStockToken) {
      ...
      const tx = await dexContract
        .connect(user1)
        .swapNGNForStock(testStockToken, swapAmount, minStockOut, deadline);
      const receipt = await tx.wait();
  ...
  testResults.push(
    await runTest("DEX - Get Swap Quote (Stock to NGN)", async () => {
      if (!testStockToken) {
        throw new Error("No test stock token available");
      }
  ...
  testResults.push(
    await runTest("DEX - Execute Swap (Stock to NGN)", async () => {
      if (!testStockToken) {
        throw new Error("No test stock token available");
      }
      ...
      const tx = await dexContract
        .connect(user1)
        .swapStockForNGN(testStockToken, swapAmount, minNGNOut, deadline);
      const receipt = await tx.wait();
  ...
  testResults.push(
    await runTest("DEX - Price Tracking", async () => {
      if (!testStockToken) {
        throw new Error("No test stock token available");
      }
  ...
  testResults.push(
    await runTest("DEX - Statistics", async () => {
      const [totalPairs, totalVolumeNGN, feesCollected, totalLiquidity] =
        await dexContract.getDEXStats();
