...
async function main() {
  ...
  try {
    ...
    const [paginatedSymbols, paginatedAddresses] = await factory.getDeployedTokensPaginated(0, 5);
    ...
    console.log(`   📈 Success rate: ${((verifiedTokens / (verifiedTokens + failedTokens)) * 100).toFixed(1)}%`);
