...
async function deployToSepolia(): Promise<DeploymentResult> {
  ...
  try {
    ...
    console.log(`   Total Batches: ${Math.ceil(deploymentResult.totalTokens / deploymentResult.batchSize)}`);
    ...
    const sectorCounts = deploymentResult.tokens.reduce((acc, token) => {
