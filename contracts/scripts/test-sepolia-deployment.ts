...
async function generateTestReport(deploymentData: DeploymentData) {
  ...
  const reportPath = resolve(
    ...
    "..",
    "test-reports",
    `sepolia-test-report-${Date.now()}.json`
  );
...
async function main() {
  ...
  try {
    ...
    await generateTestReport(deploymentData);
    ...
    console.log(
      `   Block Explorer: https://sepolia.etherscan.io/address/${deploymentData.factoryAddress}`
    );
    console.log("=".repeat(60));
  } catch (error) {
    console.error("\n❌ Test Failed:", error);
